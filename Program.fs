...
module Program =
    let solveAndVisualize () =
        ...
        // TODO: Gap von Heuristiklösung ZF Wert zu Optimallösung ZF Wert berechnen und anzeigen
    ...
    let buildTableForVaryingKappas () =
        ...
        // TODO: Ergebnisse in Tabelle bis 2.0 berechnen lassen (Stundenlang)
        for kappa in (*infty ::*) [1.7 .. 0.1 .. 2.0] do
    ...
    let trySSGS2 () =
        let testFilename = @"Projekte/Modellendogen0001.DAT"
        let ps = PSPLibParser.parse testFilename
        let (z, sts) = ps.CleverSSGS (fun r t -> 0)
        ScheduleVisualisation.showSchedules [("SSGS2", ps, sts, z)]
        ()
